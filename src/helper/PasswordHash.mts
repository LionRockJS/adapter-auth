...
function assertSupportedParameter(name: string, value: number) {
  if (!Number.isInteger(value) || value < 1) {
    throw new Error(`Invalid Argon2id ${name} parameter`);
  }
}

function parseHash(hash: string): Argon2idHash {
  ...
  if (version !== ARGON2_VERSION) throw new Error('Unsupported Argon2id hash version');
  ...
  if (memorySize > MAX_SUPPORTED_MEMORY_SIZE) throw new Error('Argon2id memory parameter is too high');
...
function encodeHash(params = ARGON2_OPTIONS, salt: Uint8Array, digest: Uint8Array) {
...
function derivePassword(text: string, salt: Uint8Array, params = ARGON2_OPTIONS) {
