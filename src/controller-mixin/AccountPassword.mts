...
import { ORM, ControllerMixinDatabase, Model } from '@lionrockjs/central';
...
export default class ControllerMixinAccountPassword extends ControllerMixin {
  ...
  static async action_change_password_post(state: Map<string, any>) {
    ...
    const { user_id } = state.get(ControllerState.REQUEST).session;

    const identifierInstances = await ORM.readBy(Identifier.Model, 'user_id', [user_id], { database , asArray:true}) as Model[];
    ...
    await Promise.all(
      identifierInstances.map(async (it: any) =>{
        try{
  ...
  static async action_change_password_done(state: Map<string, any>) {
